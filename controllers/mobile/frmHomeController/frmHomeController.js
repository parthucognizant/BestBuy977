define({ 
  ...
  getCategories : function(timeOfCalling){
    ...
    if(timeOfCalling && timeOfCalling === "initial")
    ...
    if(isNetworkAvailable() && MF.isMFInitialized){
      ...
      integrationObj.invokeOperation(MF.operations[0], "", data, function(success){controllerReference.getCategoriesSuccess(success,false,timeOfCalling);}, this.getCategoriesFailure);
    }else{
      kony.print("Categories>> Network not availeble or MF is not initialized");
      kony.timer.schedule("gettingCategories", function(){controllerReference.getCategories(timeOfCalling);}, 2, false);
    }
  },
  ...
  setCategoriesData : function(data){
    ...
    for(var i in data){
      var record = {
        "lblCategoryTitle" : {"text":data[i].subCategoryName},
  ...
  segmentRowclick : function(){
    ...
    this.onClickData = {id:this.categoryId,name:selectedData[0].lblCategoryTitle.text};
